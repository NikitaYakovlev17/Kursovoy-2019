...
namespace MFST
{
	...
	Mfst::RC_STEP Mfst::step(Log::LOG log)//( bool need_trace) //выполнить шаг автомата----------------------------------------------------?
	{
		...
		{
			if (ISNS(stack.top())) //извлекаем последний элемент стека и проверяем нетерминал ли он
			{
				GRB::Rule rule;
				if ((ruleNo = greibach.getRule(stack.top(), rule)) >= 0) //смотрим, есть ли такое правило 
				{
