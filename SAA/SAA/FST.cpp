...
namespace FST
{
	...
	{
		short* rstates = new short[fst.nstates];//буферый массив
	...
	FST* arr_of_fst_word()
	{
		FST fstDig(LEX_REAL, SYMB_LEXEMS, "", 5,
			NODE(1, RELATION('r', 1)),
			NODE(1, RELATION('e', 2)),
			NODE(1, RELATION('a', 3)),
			NODE(1, RELATION('l', 4)),
		...
		FST fstVar(LEX_DIM, SYMB_LEXEMS, "", 4,
			NODE(1, RELATION('d', 1)),
			NODE(1, RELATION('i', 2)),
			NODE(1, RELATION('m', 3)),
			NODE()
		);
		FST fstBody(LEX_ENTRY, SYMB_LEXEMS, "", 6,
			NODE(1, RELATION('e', 1)),
			NODE(1, RELATION('n', 2)),
			NODE(1, RELATION('t', 3)),
			NODE(1, RELATION('r', 4)),
		...
		FST fstStrLit(LEX_WORD, SYMB_LITERAL, "", 4,
