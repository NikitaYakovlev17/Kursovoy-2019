...
namespace ICP
{
	ICP GetIn(wchar_t infile[])
	{
		...
		char tempWord[ICP_WORD_MAX_LEN];
		short wordLength = 0;

		while ((!inFile.eof()) && (tempChar = inFile.get())) {
			if (wordLength >= ICP_WORD_MAX_LEN)
			{
			...
			if (tempChar == ICP_CODE_SPACE || inStruct.code[(int)tempChar] == ICP::S || tempChar == ICP_CODE_ENDL) 
			{
				...
				if (tempChar == ICP_CODE_SPACE)
				{
					inStruct.ignor += 1;
				}

				if (inStruct.code[(int)tempChar] == ICP::S)
				{
					char word[2];
					word[0] = tempChar;
					word[1] = '\0';
			...
			else if (inStruct.code[(int)tempChar] != ICP::F)
			{
				positionInTheString++;
				tempWord[wordLength++] = tempChar;
				inStruct.size++;
			...
			if (inStruct.size >= ICP_MAX_LEN_TEXT)
			{
