...
namespace Lex
{
	...
	LEX Lexer(ICP::ICP in, Log::LOG log)
	{
		...
		string func_name = "", funcHesisName = ""; //имя текущей функции (для области видимости), имя функции (для семантического анализа её параметров)
		...
		for (int i = 0; i < size; i++) {
			...
			case FST::SYMB_LEXEMS:
			{
				...
				case LEX_DIM:
					...
					{
						throw ERROR_THROW_IN(600, elementLT.lineNo - 1, -1);
					}
					break;
